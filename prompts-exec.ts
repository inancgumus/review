...
export const execPrompts: PromptSet = {
	buildReviewPrompt: execReviewPrompt,
	buildFixPrompt: execFixPrompt,
};

function execReviewPrompt(p: ReviewPromptParams): string {
...
function execIncrementalPrompt(round: number): string {
	return [
		`Re-check round ${round}. The implementer worked on the step you assigned (see the summary above).`,
...
function execFullPrompt(p: ReviewPromptParams): string {
	const parts = [
		"You are an implementation orchestrator. You oversee a plan and direct an implementer to build it step by step.",
		`Focus: ${p.focus}`,
		"",
		...
		"2. If the implementer just attempted a step: read every file they claimed to change. Run tests. If anything is missing, wrong, or broken — reassign the same step with specific feedback on what failed.",
		"3. Only after verifying the current step is FULLY correct, identify the next unimplemented step.",
		"4. Describe EXACTLY what the implementer should do for this ONE step — file paths, function signatures, behavior, tests.",
		"",
		"RULES:",
		...
		"- Your output goes directly to the implementer. ONLY describe the current step.",
		"- Do NOT mention future steps, the overall plan, or what comes next. The implementer must not know the full plan.",
		"- Be specific and actionable. The implementer should not have to guess.",
		"",
		"VERIFICATION CHECKLIST (do this every round):",
		"- Read the files the implementer changed. Do they match what was asked?",
		...
		"VERDICT: APPROVED",
		"",
		"If the implementer needs to do work (implement next step or redo current):",
		"VERDICT: CHANGES_REQUESTED",
	];
	...
	if (p.round > 1 && p.fixerSummaries.length > 0) {
		parts.push(
			"",
			"## Previous rounds",
			"Below is the implementer's SELF-REPORTED summary. Do NOT trust it. Verify by reading the actual code.",
			"",
			...p.fixerSummaries,
			"",
			"## YOUR JOB THIS ROUND",
			"You MUST read the actual source files and run tests/build before giving a verdict.",
			"Do NOT trust the summary above. The implementer may have cut corners, skipped tests, or left broken code.",
...
function execFixPrompt(reviewText: string, _contextPaths: string[], round: number): string {
	const cleaned = sanitize(reviewText.replace(/VERDICT:\s*\*{0,2}CHANGES_REQUESTED\*{0,2}/gi, "").trim());
	return [
		`## Implementation Task — Round ${round}`,
		"",
		cleaned,
		"",
		"---",
		...
		"",
		"IMPORTANT: Do NOT output any VERDICT lines. You are the implementer, not the orchestrator.",
		"",
