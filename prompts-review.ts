...
function reviewFullPrompt(p: OverseerPromptParams): string {
	const parts = [
		...
		"1. **Root commit** — use `git log --oneline`, `git log -p`, or `git blame` to trace the issue back to the commit that INTRODUCED it. Don't just report where the symptom shows up — find where it originated. Fixing at the root avoids unnecessary diff churn across later commits.",
		"2. **File and line** — exact location in that originating commit",
		"3. **What's wrong** — be specific, not vague",
		"4. **How to fix it** — concrete suggestion targeting the root commit, not a downstream patch",
		"",
...
function reviewWorkhorsePrompt(overseerText: string, contextPaths: string[], round: number): string {
	...
	return [
		...
		"  The overseer traced each issue to its originating commit. Fix each issue IN that root commit, not in a later one.",
		"  1. Fix each issue, then stage ONLY its files: `git add <files>`",
		"  2. Create a fixup commit targeting the ROOT SHA the overseer identified: `git commit --fixup=<sha>`",
